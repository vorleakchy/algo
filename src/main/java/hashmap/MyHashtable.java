...
public class MyHashtable implements Iterable {
	...
	public MyHashtable(int tableSize) {
		...
		keyTable = new LinkedList[tableSize];
	}
	...
	public boolean containsKey(Object key){
		...
		if(keyTable[hash] == null) return false;
		LinkedList keyList = keyTable[hash];
	...
	public void put(Object key, Object value){
		if(key==null) return;
		//insert key into keyTable
		int hashcode = key.hashCode();
		int hash = hash(hashcode);
		if(keyTable[hash] == null) {
			keyTable[hash] = new LinkedList();
		}
		keyTable[hash].add(key);
	...
	public Object remove(Object key){
		...
		while(it.hasNext()){
			...
			if(e.key.equals(key)) {
				...
				indexKeyTable = keyTable[hash].indexOf(key);
				value = e.value;
		...
		if(indexTable == -1) return null;
			...
			keyTable[hash].remove(indexKeyTable);
			return value;
		}
